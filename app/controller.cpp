...
void appliParam(){
	...
	//CH2
	if(data.AV1){
		HAL_GPIO_WritePin(AV_1_GPIO, AV_1, GPIO_PIN_SET);
	}else{
		HAL_GPIO_WritePin(AV_1_GPIO, AV_1, GPIO_PIN_SET);
	}
	switch(data.Amp1){
...
void saveParam(){
	...
	sdCard.saveParam(4,data.AV1);
	sdCard.saveParam(5,data.Amp1);
...
void saveData(uint8_t adc){
	...
	if(adc==1){
		for(uint16_t x=0; x<data.numRankADC1; x++){
			...
			sdCard.saveData(num,dataValue[x]);
		}
	}
	else if(adc == 3){
		for(uint16_t x=0; x<data.numRankADC3; x++){
			...
			sdCard.saveData(num+sizeof(data.adc1CH),dataValue[x]);
		}
	}
	dataV = 0;
}
