...
void cardSDRW::openData()
{
	...
	if(data->numAdcCH[0]){
		//adcA
		sprintf(text,"0_%d_div0.smp",data->adcAnbSeries);
		...
		if(f_write(SDFile[0],(uint8_t*) &data->Amp1, 1, &byteswritten) != FR_OK)
			Error_Handler();
		if(f_write(SDFile[0],(uint8_t*) &data->adcAPeriod, 4, &byteswritten) != FR_OK)
			Error_Handler();
		if(f_write(SDFile[0],(uint8_t*) &data->adcADiv, 1, &byteswritten) != FR_OK)
			Error_Handler();
	}

	if(data->numAdcCH[1]){
		//adc3
		sprintf(text,"1_%d_div0.smp",data->adcBnbSeries);
		...
		if(f_write(SDFile[1],(uint8_t*) &data->Amp3, 1, &byteswritten) != FR_OK)
			Error_Handler();
		if(f_write(SDFile[1],(uint8_t*) &data->adcBPeriod, 2, &byteswritten) != FR_OK)
			Error_Handler();
		if(f_write(SDFile[1],(uint8_t*) &data->nullVal,2, &byteswritten) != FR_OK)
					Error_Handler();
		if(f_write(SDFile[1],(uint8_t*) &data->adcBDiv, 1, &byteswritten) != FR_OK)
			Error_Handler();
	}
}
...
void cardSDRW::deleteData(uint8_t nbVersTM1, uint8_t nbVersTM2){
	char text [20];
	for (uint8_t x = 0; x < nbVersTM1; ++x) {
		for(uint8_t i = 0; i<4; i++){
			if(sizeData(0,x,i)!= 0){
				sprintf(text,"0_%d_div%d.smp",x,i);
				if( f_unlink (text) != FR_OK)
					Error_Handler();
			}
		}
	}
	for (uint8_t x = 0; x < nbVersTM2; ++x) {
		for(uint8_t i = 0; i<4; i++){
			if(sizeData(1,x,i)!= 0){
				sprintf(text,"1_%d_div%d.smp",x,i);
				if( f_unlink (text) != FR_OK)
					Error_Handler();
			}
		}
